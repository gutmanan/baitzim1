...
public class Customer {
		...
		for (int i = 0; i < subs.length; i++) {
			if (subs[i] != null) {
				for (int j = 0; j < subs[i].getRoomRuns().length; j++) {
					...
					if (l != null && l.getStartDateTime().before(subs[i].getLastDay())) {
	...
	public boolean addRoomRun(RoomRun roomRunToAdd) {
		if (roomRunToAdd != null) {
			for (int i = 0; i < subs.length; i++) {
				if (subs[i] != null && roomRunToAdd.getStartDateTime().before(subs[i].getLastDay()) && roomRunToAdd.getStartDateTime().after(subs[i].getStartDate())) {
					for (RoomRun l : subs[i].getRoomRuns()) {
						if (l != null && roomRunToAdd.equals(l)) {
							return false;
						}
					}
					if (roomRunToAdd.getRoom().getBranch().equals(subs[i].getReceptionist().getWorkBranch()))
