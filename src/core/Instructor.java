...
public class Instructor extends Employee {
	...
	public boolean addRoomRun(RoomRun roomRun) {
		if (roomRun == null || roomRun.getLevel().getLevel() > this.Level)
			return false;
		for (RoomRun rr : roomRuns) {
			if (rr != null) {
				...
				if (!checkType)
					return false;
				long minutesDiff = TimeUnit.MINUTES
						.convert(roomRun.getStartDate().getTime() - rr.getStartDate().getTime(), TimeUnit.MILLISECONDS);
	 ...
	 */
	public boolean removeLesson(RoomRun roomRun) {
